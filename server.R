server = function(input, output, session) {
  ...
  output$gifter = renderText({paste("Welcome back,", to_title_case(res_auth$user))})
  ...
  observeEvent(input$update_wishlist, {
    ...
    others_wishlist() %>%
      rbind(tibble::tibble(receiver = res_auth$user, wishlist = user_wishlist())) %>%
      write_db_encrypt(con, value = ., name = "wishlist", passphrase = DB_PASSPHRASE)
  })
